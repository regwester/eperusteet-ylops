...
ylopsApp.controller('JarjestelmaVirheModalController', function ($scope, $modalInstance, $state, viesti) {
    $scope.viesti = viesti;
    $scope.ok = $modalInstance.close;
  })
  .service('Notifikaatiot', function ($rootScope, $timeout, NOTIFICATION_DELAY_SUCCESS,
    NOTIFICATION_DELAY_WARNING, $modal, $state, Kaanna) {
    var viestit = [];
    ...
    function fataali(viesti, cb) {
      ...
      $modal.open({
        templateUrl: 'views/common/modals/jarjestelmavirhe.html',
    ...
    function serverCb(response) {
      if (response) {
        if (response.status >= 500) {
          fataali(Kaanna.kaanna('jarjestelmavirhe-teksti', {virhekoodi: response.status}), function () {
