...
interface GenericTreeConfig<NodeType extends GenericTreeNode> {
  root: () => IPromise<NodeType>
  children: (node: NodeType) => IPromise<NodeType[]>
  useUiSortable: () => boolean
  acceptDrop?: (from: NodeType, to:NodeType, event: any, ui: any) => boolean
  sortableClass?: (node: NodeType) => string
  template?: (node: NodeType) => string
  hidden?: (node: NodeType) => boolean
  extension: (node: any /*Niko: Mikä tämän edes on tarkoitus olla?*/, $scope: any) => void
}
