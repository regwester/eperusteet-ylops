...
ylopsApp
    .service("TekstikappaleOps", function(
        $state,
        $stateParams,
        ...
        function deleteKappale(model, osio, opsId, kappale, cb) {
            ...
            OpetussuunnitelmanTekstit.delete(
                params,
                kappale,
                function() {
                    var foundIndex = null,
        ...
        this.rakennaSivunavi = (tekstit, isPohja) => {
            const state = isPohja
                ? "root.pohjat.yksi.sisalto.tekstikappale"
                : "root.opetussuunnitelmat.yksi.sisalto.tekstikappale";

            return _(
                _.deepFlatten(tekstit, _.property("lapset"), (obj, depth) => {
    ...
    .controller("OpetussuunnitelmaSisaltoController", function(
        ...
        $scope.tekstitProvider = $q.when({
            ...
            acceptDrop: (node, target, parentScope) => {
                return (
                    target !== $scope.model.tekstit ||
                    parentScope === $scope ||
                    (!_.isEmpty(target.tekstiKappale) && $scope.isLukio) ||
                    node.$$depth === 0
                );
            },
            sortableClass: node => {
    ...
    .controller("OpsAlihierarkiaModalController", function(
        $scope,
        $modalInstance,
        ops,
        aliOpsit,
