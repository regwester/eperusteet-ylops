...
ylopsApp.service("VuosiluokatService", function(
    $q,
    $state,
    OppiaineCRUD,
    Utils,
    ...
    function getVuosiluokkakokonaisuus(opetussuunnitelmaId, vlkId, successCb, errorCb) {
        return VuosiluokkakokonaisuusCRUD.get(
            { opsId: opetussuunnitelmaId, vlkId: vlkId },
    ...
    function getLaajaalaiset(opetussuunnitelmaId) {
        var deferred = $q.defer();
        OpetussuunnitelmaCRUD.laajaalaiset(
            { opsId: opetussuunnitelmaId },
            function(res) {
    ...
    function getPerusteOppiaine(oppiaineId) {
        var deferred = $q.defer();
        OppiaineCRUD.peruste(
            { opsId: opsId || OpsService.getId() },
            { id: oppiaineId },
            function(res) {
                deferred.resolve(res);
            },
