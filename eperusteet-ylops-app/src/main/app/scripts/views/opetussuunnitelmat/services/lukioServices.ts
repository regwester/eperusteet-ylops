...
interface LukioOpetussuunnitelmaServiceI {
    ...
    updateAihekokonaisuudetYleiskuvaus(
        yleiskuvaus: Lukio.AihekokonaisuudetPaivitaYleiskuvaus,
        opsId?: number
    ): IPromise<void>;
    ...
    addKielitarjonta(
        oppiaineId: number,
        tarjonta: Lukio.OppiaineKielitarjonta,
        opsId?: number
    ): IPromise<Lukio.IdHolder>;
...
ylopsApp.service("LukioOpetussuunnitelmaService", function(
    OpetusuunnitelmaLukio,
    $q: IQService,
    $log,
    Lukko,
    Notifikaatiot,
    ...
    const doGetAihekokonaisuudet = (opsId: any, d: IDeferred<Lukio.AihekokonaisuudetPerusteenOsa>) =>
        OpetusuunnitelmaLukio.aihekokonaisuudet({ opsId: opsId || $stateParams.id }).$promise.then(
            (aihekok: Lukio.AihekokonaisuudetPerusteenOsa) => d.resolve(aihekok),
    ...
    const lukitseAihekokonaisuudet = (id: number, opsId?: number) =>
        Lukko.lockLukio({
            opsId: opsId || $stateParams.id,
            id: id,
            lukittavaOsa: "AIHEKOKONAISUUDET"
        });
    const vapautaAihekokonaisuudet = (id: number, opsId?: number) =>
        Lukko.unlockLukio({
            opsId: opsId || $stateParams.id,
            id: id,
            lukittavaOsa: "AIHEKOKONAISUUDET"
    ...
    const rearrangeAihekokonaisuudet = (jarjestys: Lukio.JarjestaAihekokonaisuudet, opsId?: number) =>
        OpetusuunnitelmaLukio.rearrangeAihekokonaisuudet(
            { opsId: opsId || $stateParams.id },
            jarjestys
        ).$promise.then(r => {
            aiheKokCache.clear();
    ...
    const updateAihekokonaisuudetYleiskuvaus = (
        yleiskuvaus: Lukio.AihekokonaisuudetPaivitaYleiskuvaus,
        opsId?: number
    ) =>
        OpetusuunnitelmaLukio.updateAihekokonaisuudetYleiskuvaus(
            { opsId: opsId || $stateParams.id },
            yleiskuvaus
        ).$promise.then(r => {
            aiheKokCache.clear();
    ...
    const kokonaisuusCache = aiheKokCache.related((from: Lukio.AihekokonaisuudetPerusteenOsa): {
        [key: number]: Lukio.OpsAihekokonaisuus;
    } =>
        _(from.paikallinen.aihekokonaisuudet)
            .indexBy(_.property("id"))
            .value()
    );
    const getAihekokonaisuus = (id: number, opsId?: number) => kokonaisuusCache.get(opsId || $stateParams.id, id);
    const lukitseAihekokonaisuus = (id: number, opsId?: number) =>
        Lukko.lockLukio({
            opsId: opsId || $stateParams.id,
            id: id,
            lukittavaOsa: "AIHEKOKONAISUUS"
        });
    const vapautaAihekokonaisuus = (id: number, opsId?: number) =>
        Lukko.unlockLukio({
            opsId: opsId || $stateParams.id,
            id: id,
            lukittavaOsa: "AIHEKOKONAISUUS"
    ...
    const saveAihekokonaisuus = (aihekok: Lukio.LuoAihekokonaisuus, opsId?: number) =>
        OpetusuunnitelmaLukio.saveAihekokonaisuus({ opsId: opsId || $stateParams.id }, aihekok).$promise.then(r => {
            kokonaisuusCache.clear();
            return r;
        }, Notifikaatiot.serverCb);
    const updateAihekokonaisuus = (id: number, aihekok: Lukio.PaivitaAihekokonaisuus, opsId?: number) =>
        OpetusuunnitelmaLukio.updateAihekokonaisuus(
            { opsId: opsId || $stateParams.id, aihekokonaisuusId: id },
            aihekok
        ).$promise.then(r => {
            aiheKokCache.clear();
            return r;
    ...
    const deleteAihekokonaisuus = (aihekokId: number, opsId?: number) =>
        OpetusuunnitelmaLukio.deleteAihekokonaisuus({
            opsId: opsId || $stateParams.id,
    ...
    const doGetOpetuksenYleisetTavoitteet = (opsId: any, d: IDeferred<Lukio.OpetuksenYleisetTavoitteetPerusteenOsa>) =>
        OpetusuunnitelmaLukio.opetuksenYleisetTavoitteet({ opsId: opsId || $stateParams.id }).$promise.then(
            (yleisetTavoitteet: Lukio.OpetuksenYleisetTavoitteetPerusteenOsa) => d.resolve(yleisetTavoitteet),
            Notifikaatiot.serverCb
        );
    const yleisetTavoitteetCache = cached<Lukio.OpetuksenYleisetTavoitteetPerusteenOsa>(
        $q,
    ...
    const lukitseYleisetTavoitteet = (opsId?: any): IPromise<void> =>
        Lukko.lockLukio({
            opsId: opsId || $stateParams.id,
            id: opsId || $stateParams.id,
            lukittavaOsa: "YLEISET_TAVOITTEET"
        });
    const vapautaYleisetTavoitteet = (opsId?: any): IPromise<void> =>
        Lukko.unlockLukio({
            opsId: opsId || $stateParams.id,
            id: opsId || $stateParams.id,
            lukittavaOsa: "YLEISET_TAVOITTEET"
    ...
    const updateYleisetTavoitteet = (tavoitteet: Lukio.OpetuksenYleisetTavoitteetUpdate, opsId?: any): IPromise<void> =>
        OpetusuunnitelmaLukio.updateOpetuksenYleisetTavoitteet(
            { opsId: opsId || $stateParams.id },
            tavoitteet
        ).$promise.then(r => {
    ...
    const doGetRakenne = (id: number, d: IDeferred<Lukio.LukioOpetussuunnitelmaRakenneOps>) =>
        OpetusuunnitelmaLukio.rakenne({ opsId: id }).$promise.then(
            (rakenne: Lukio.LukioOpetussuunnitelmaRakenneOps) => d.resolve(rakenne),
            Notifikaatiot.serverCb
        );
    const rakenneCache = cached<Lukio.LukioOpetussuunnitelmaRakenneOps>($q, doGetRakenne);
    const lukitseRakenne = (opsId?: number) =>
        Lukko.lockLukio({
            opsId: opsId || $stateParams.id,
            id: opsId || $stateParams.id,
            lukittavaOsa: "OPS"
        });
    const vapautaRakenne = (opsId?: number) =>
        Lukko.unlockLukio({
            opsId: opsId || $stateParams.id,
            id: opsId || $stateParams.id,
            lukittavaOsa: "OPS"
    ...
    const updateOppiaineKurssiStructure = (treeRoot, kommentti?: string, opsId?: number) => {
        var chain = _(treeRoot).flattenTree((node) => {
            ...
            update = {
                ...
                kurssit: chain
                    .filter(n => n.dtype == LukioKurssiTreeNodeType.kurssi)
                    .reducedIndexOf(
                        n => n.id,
                        (a, b) => {
                            var c = _.clone(a);
    ...
    const doGetOppiaine = (oppiaineWithOpsId: number | string, d: IDeferred<Lukio.LukioOppiaine>) => {
        ...
        return OpetusuunnitelmaLukio.oppiaine({ opsId: opsId, oppiaineId: oppiaineId }).$promise.then(
            (oa: Lukio.LukioOppiaine) => d.resolve(oa),
            Notifikaatiot.serverCb
        );
    };
    ...
    const lukitseOppiaine = (id: number, opsId?: number) =>
        Lukko.lockLukio({
            opsId: opsId || $stateParams.id,
            id: id,
            lukittavaOsa: "OPPIAINE"
        });
    const vapautaOppiaine = (id: number, opsId?: number) =>
        Lukko.unlockLukio({
            opsId: opsId || $stateParams.id,
            id: id,
            lukittavaOsa: "OPPIAINE"
    ...
    const saveOppiaine = (oppiaine: Lukio.LukioOppiaineTallennus, opsId?: number) =>
        OpetusuunnitelmaLukio.saveOppiaine({ opsId: opsId || $stateParams.id }, oppiaine).$promise.then(r => {
            oppiaineCache.clear();
            return r;
        }, Notifikaatiot.serverCb);
    const kloonaaOppiaineMuokattavaksi = (oppiaineId: number, opsId?: number) =>
    ...
    const palautaYlempaan = (oppiaineId: number, opsId?: number) =>
    ...
    const updateOppiaine = (oppiaine: Lukio.LukioOppiaineTallennus, opsId?: number) =>
        OpetusuunnitelmaLukio.updateOppiaine({ opsId: opsId || $stateParams.id }, oppiaine).$promise.then(r => {
            oppiaineCache.clear();
            return r;
        }, Notifikaatiot.serverCb);
    const addKielitarjonta = (oppiaineId: number, tarjonta: Lukio.OppiaineKielitarjonta, opsId?: number) =>
        OpetusuunnitelmaLukio.addKielitarjonta(
            { oppiaineId: oppiaineId, opsId: opsId || $stateParams.id },
            tarjonta
        ).$promise.then(r => {
            oppiaineCache.clear();
            return r;
    ...
    const addAbstraktiOppiaine = (tarjonta: Lukio.AbstraktiOppiaine, opsId?: number) =>
        OpetusuunnitelmaLukio.addAbstraktiOppiaine({ opsId: opsId || $stateParams.id }, tarjonta).$promise.then(r => {
            oppiaineCache.clear();
            return r;
        }, Notifikaatiot.serverCb);
    const deleteOppiaine = (oppiaineId: number, opsId?: number) =>
        OppiaineCRUD.delete({ oppiaineId: oppiaineId, opsId: opsId || $stateParams.id }).$promise.then(() => {
            oppiaineCache.clear();
        }, Notifikaatiot.serverCb);
    ...
    const lukitseKurssi = (id: number, opsId?: number) =>
        Lukko.lockLukio({
            opsId: opsId || $stateParams.id,
            id: id,
            lukittavaOsa: "LUKIOKURSSI"
        });
    const vapautaKurssi = (id: number, opsId?: number) =>
        Lukko.unlockLukio({
            opsId: opsId || $stateParams.id,
            id: id,
            lukittavaOsa: "LUKIOKURSSI"
    ...
    const saveKurssi = (kurssi: Lukio.LuoLukiokurssi, opsId?: number) =>
        OpetusuunnitelmaLukio.saveKurssi({ opsId: opsId || $stateParams.id }, kurssi).$promise.then(r => {
            kurssiCache.clear();
            return r;
        }, Notifikaatiot.serverCb);
    const updateKurssi = (kurssiId: number, kurssi: Lukio.UpdateLukiokurssi, opsId?: number) =>
        OpetusuunnitelmaLukio.updateKurssi(
            { kurssiId: kurssiId, opsId: opsId || $stateParams.id },
            kurssi
        ).$promise.then(r => {
            kurssiCache.clear();
            return r;
    ...
    const disconnectKurssi = (kurssiId: number, oppiaineId: number, opsId: number) =>
        OpetusuunnitelmaLukio.disconnectKurssi(
            { kurssiId: kurssiId, opsId: opsId || $stateParams.id, oppiaineId: $stateParams.oppiaineId },
            {}
        ).$promise.then(r => {
            kurssiCache.clear();
            return r;
    ...
    const reconnectKurssi = (kurssiId: number, oppiaineId: number, opsId: number) =>
        OpetusuunnitelmaLukio.reconnectKurssi(
            { kurssiId: kurssiId, opsId: opsId || $stateParams.id, oppiaineId: $stateParams.oppiaineId },
            {}
        ).$promise.then(r => {
            kurssiCache.clear();
    ...
    const removeKurssi = (kurssiId: number, opsId: number) =>
        OpetusuunnitelmaLukio.removeKurssi(
            { kurssiId: kurssiId, opsId: opsId || $stateParams.id },
            {}
        ).$promise.then(r => {
            kurssiCache.clear();
