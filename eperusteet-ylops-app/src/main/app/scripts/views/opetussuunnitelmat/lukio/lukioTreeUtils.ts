...
interface LukioTreeUtilsI {
    ...
    extensions: (
        ...
        myExtensions?: ((node: LukioKurssiTreeNode, scope: any) => void)
    ) => ((node: LukioKurssiTreeNode, scope: any) => void);
    acceptDropWrapper: (state: LukioKurssiTreeState) => (node: LukioKurssiTreeNode, to: LukioKurssiTreeNode) => boolean;
...
ylopsApp.service("LukioTreeUtils", function($log, $state, $stateParams, Kaanna, Kieli, $timeout, $rootScope, $filter) {
    ...
    function removeKurssiFromOppiaine(state: LukioKurssiTreeState, node: LukioKurssiTreeNode) {
        ...
        var kurssitInTree = _(state.root())
                .flattenTree(n => n.lapset)
                .filter(c => c.dtype == LukioKurssiTreeNodeType.kurssi && c.id == node.id)
                .value(),
    ...
    var templatesByState = (state: LukioKurssiTreeState) => {
        var templateAround = (tmpl: JQuery, node) => {
            var classes =
                    "tree-list-item " +
                    (node.$$depth === 0 ? "opetussisaltopuu-solmu-paataso " : "") +
        ...
        var kurssiListaTemplateAround = (tmpl: JQuery, node) =>
            $(
                '<span class="span-container liittamaton-kurssi recursivetree tree-list-item bubble-osa empty-item"></span>'
            ).append(tmpl);
        var templates = <LukioTreeTemplatesI>{
            treeHandle: node => {
                return state.isEditMode()
                    ? $(
                          '<span icon-role="drag" class="treehandle"><span class="glyphicon glyphicon-resize-vertical"></span></span>'
                      )
                    : null;
            },
            ...
            name: node => {
                ...
                if (node.dtype == LukioKurssiTreeNodeType.kurssi) {
                    var lokalisoituKoodi = $filter("kaanna")(node.lokalisoituKoodi),
                        name =
                            $filter("kaanna")(node.nimi) +
                            " " +
                            (lokalisoituKoodi ? " (" + lokalisoituKoodi + ")" : "");
            ...
            collapse: (n: LukioKurssiTreeNode) =>
                (!state.isEditMode() || n.dtype == LukioKurssiTreeNodeType.oppiaine) && n.lapset.length
                    ? $('<span class="colorbox collapse-toggle"></span>')
                          .append(
                              $('<span ng-hide="node.$$collapsed" class="glyphicon collapse-based"></span>').addClass(
                                  n.$$collapsed ? "glyphicon-chevron-right" : "glyphicon-chevron-down"
                              )
                          )
                          ...
                          .click(e => toggleNode(n, e, state))
                    : null,
            nodeTemplate: (n: LukioKurssiTreeNode) => {
                if (n.dtype == LukioKurssiTreeNodeType.kurssi) {
                    var remove = state.isEditMode()
                        ? $(
                              '<span class="remove" icon-role="remove"><span class="glyphicon glyphicon-remove"></span></span>'
                          ).click(e => removeKurssiFromOppiaine(state, n))
                        : null;
                    return templateAround(
                        $('<span class="span-container puu-node kurssi-node"></span>')
                            ...
                            .append(
                                $('<span class="span-container node-content left">')
                    ...
                    return templateAround(
                        $('<span class="span-container puu-node oppiaine-node">')
                            .append(templates.treeHandle(n))
                            .append(templates.collapse(n))
                            .append(templates.timestamp(n))
                            .append(
                                $('<span class="span-container node-content left"></span>')
                                    .addClass(_.isEmpty(n.lapset) ? "empty-node" : "")
                                    .append($("<strong></strong>").append(templates.name(n)))
                            ),
                        n
                    );
                }
            ...
            nodeTemplateKurssilista: (n: LukioKurssiTreeNode) =>
                kurssiListaTemplateAround(
                    $('<span class="span-container puu-node kurssi-node"></span>')
                        .append(templates.treeHandle(n))
                        .append(templates.kurssiColorbox(n))
                        .append(templates.timestamp(n))
                        .append($('<span class="span-container node-content left"></span>').append(templates.name(n))),
                    n
                )
        };
        return templates;
    ...
    var defaultHidden = usePagination => (node: LukioKurssiTreeNode) =>
        !node ||
        node.$$hide ||
        (node.$$nodeParent && node.$$nodeParent.$$collapsed) ||
        (usePagination && !node.$$pagingShow);
    var defaultCollapsed = (node: LukioKurssiTreeNode) =>
        node &&
        (node.$$collapsed || (node.$$nodeParent && node.$$nodeParent.$$collapsed)) &&
    ...
    var updateCollapse = (state: LukioKurssiTreeState) => {
        doSetCollapse(
            ...
            n =>
                state.isEditMode()
                    ? state.defaultCollapse ? !(n.dtype == LukioKurssiTreeNodeType.oppiaine && n.koosteinen) : false
    ...
    var acceptMove = function(node: LukioKurssiTreeNode, to: LukioKurssiTreeNode) {
        ...
        if (
            ...
            !to.koosteinen &&
            (!node.$$nodeParent || node.$$nodeParent.id !== to.id) &&
            _.any(to.lapset, kurssi => kurssi.dtype == LukioKurssiTreeNodeType.kurssi && kurssi.id === node.id)
        ) {
            return false;
        }
    ...
    var moveWrapper = (state: LukioKurssiTreeState) => (node: LukioKurssiTreeNode, to: LukioKurssiTreeNode) => {
