...
ylopsApp
    ...
    .controller("TekstikappaleController", function(
        $scope,
        $q,
        Editointikontrollit,
        Notifikaatiot,
        ...
        const updateMuokkaustieto = () => {
            OpetussuunnitelmanTekstit.versiot(
                { id: $stateParams.id, tekstiId: $scope.model.tekstiKappale.id },
                {},
                res => {
                    $scope.versiot = { list: [] };
                    ...
                    if ($scope.versiot.latest) {
                        ...
                        $scope.versiot.chosen = _.find($scope.versiot.list, {
                            numero: parseInt($stateParams.versio.replace("/", ""))
                        });
                    }
                }
            );
        ...
        $scope.kopioiMuokattavaksi = function() {
            Varmistusdialogi.dialogi({
                ...
                successCb: function() {
                    Lukko.lock(commonParams, function() {
                        OpetussuunnitelmanTekstit.kloonaaTekstikappale(
                            commonParams,
                            {},
                            function(res) {
                                successCb(res);
        ...
        var successCb = function(res) {
            return $q((resolve, reject) => {
                ...
                if ($stateParams.tekstikappaleId === "uusi") {
                    ...
                    var navigaatiomuutos =
                        !_.isEqual(originalOtsikko, _.omit(res.tekstiKappale.nimi, "$$validointi")) ||
    ...
    .directive("valmiusIkoni", function() {
        return {
            ...
            template:
                "<span ng-attr-title=\"{{(model.valmis ? 'valmis' : 'luonnos') | kaanna}}\" " +
