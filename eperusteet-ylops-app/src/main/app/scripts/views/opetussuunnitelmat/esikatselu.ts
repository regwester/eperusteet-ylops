...
ylopsApp
    ...
    .controller("EsikatseluController", function(
                            ...
                            .map(function(vuosiluokka) {
                                return [
                                    ...
                                    _($scope.model.oppiaineet)
                                        ...
                                        .filter(function(x) {
                                            var oavlk = _.find(
                                                x.vuosiluokkakokonaisuudet,
                                                _.equals(vlk._tunniste, "_vuosiluokkakokonaisuus")
                                            );
                                            return (
                                                oavlk && _.any(oavlk.vuosiluokat, _.equals(vuosiluokka, "vuosiluokka"))
                                            );
                                        })
        ...
        $q
            .all(
                _($scope.vuosiluokkakokonaisuudet)
                    .map(function(vlk) {
    ...
    .controller("EsikatseluVlkController", function(
        $scope,
        $stateParams,
        baseLaajaalaiset,
        Utils,
    ...
    .controller("EsikatseluOppiaineController", function(
        $scope,
        $stateParams,
        perusteOppiaine,
        Kaanna,
        ...
        $scope.vlkFilter = function(vlk) {
            return !_.isEmpty(
                _.filter(vlk.vuosiluokat, function(vl) {
        ...
        _.each($scope.oppiaine.vuosiluokkakokonaisuudet, function(opVlk) {
            ...
            _.each(opVlk.vuosiluokat, function(vuosiluokka) {
                ...
                VuosiluokkaMapper.mapModel(
                    $scope.vuosiluokkaSisallot[opVlk._vuosiluokkakokonaisuus][vuosiluokka.vuosiluokka]
                );
                VuosiluokkaMapper.mapSisaltoalueet(
                    $scope.vuosiluokkaSisallot[opVlk._vuosiluokkakokonaisuus][vuosiluokka.vuosiluokka],
                    "sisaltoaluetunnisteet",
                    "sisaltoaluemuokattavat"
                );
            });
        });
    });
